...
function App() {
  const [courses] = useState([
      ...
      { id: 4, 
        name: 'KR Hoodie', 
        price: 799, 
        image:'https://tse4.mm.bing.net/th?id=OIP.FmqbQNwPvusLFJzHJxQgGQHaFj&pid=Api&P=0&h=180'
      },
      { id: 5, 
        name: 'KR shoes', 
        price: 2999, 
    ...
    const addCourseToCartFunction = (KRcourse) => {
      const alreadyCourses = cartCourses
                             .find(item => item.product.id === KRcourse.id);
      if (alreadyCourses) {
          const latestCartUpdate = cartCourses.map(item =>
              item.product.id === KRcourse.id ? { 
              ...item, quantity: item.quantity + 1 } 
              : item
          );
          setCartCourses(latestCartUpdate);
      } else {
          setCartCourses([...cartCourses, {product: KRcourse, quantity: 1}]);
      }
  };
  const deleteCourseFromCartFunction = (KRcourse) => {
    const updatedCart = cartCourses
                        .filter(item => item.product.id !== KRcourse.id);
